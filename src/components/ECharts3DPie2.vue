  ...
  setup(props) {
    ...
    const getPie3D = (pieData) => {
      ...
      series.forEach((item, index) => {
        ...
        labelAngles.push({
          start: finalRadian - angleRange * 0.5,
          end: finalRadian + angleRange * 0.5
        });
        labelPositions.push({ x: finalX, y: finalY, angle: finalRadian });

        // 计算指示线路径（优化转折点和终点，提升Z轴高度）
        const turningPosArr = [
          finalX * 1.2,   // 第一转折点（稍外扩）
          finalY * 1.2,
          finalZ * 0.8    // 中间高度提升，确保在扇形上方
        ];

        const endPosArr = [
          finalX * 2,   // 标签终点（显著外扩）
          finalY * 2,
          finalZ * 1.8   // 最高处提升，确保标签在最上方
        ];
        ...
        if (showLabel) {
          linesSeries.push(
            // 指示线（带平滑曲线，提升Z轴层级）
            {
              ...
              data: [[posX, posY, posZ * 0.6], turningPosArr, endPosArr]
            },
            // 标签（带阴影和背景，提升Z轴层级）
            {
              type: 'scatter3D',
              z: 50, // 最高Z轴层级，确保标签在最上方
              label: {
                show: true,
                distance: 0,
                // backgroundColor: 'rgba(30, 40, 70, 1)',
                // borderColor: item.pieData.itemStyle.color,
                borderWidth: 1,
                textStyle: {
                  color: '#fff',
                  fontSize: 12,
              ...
              data: [{
                ...
                value: endPosArr,
                valueData: percent,
                itemStyle: { color: item.pieData.itemStyle.color }
              }]
            }
          );
        }
    ...
    const initChart = () => {
      ...
      const option = {
        ...
        animation: true,
        labelLine: { show: false, lineStyle: { color: '#7BC0CB' } },
        label: { show: false },
        xAxis3D: {
          min: -1.5, // 扩大x轴范围
          max: 1.5,
