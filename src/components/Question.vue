            ...
            <div class="seats-bg margin-tb-xs ">
                <el-row class="row-bg" justify="space-between">
                    <el-col :span="8" @click="showModal" v-for="item in seatData" :key="item.type"
                        class="pointer padding-top-xs flex align-center justify-center "
                        >
                    
                            <img :src="item.icon" :alt="item.alt">
                            <div class="text-left margin-left-xs">
                                <div class="Roboto seats-title">{{ item.count || 0 }}<span
...
export default {
    ...
    setup() {
        ...
        const mapDataToChart = (data) => {
            seatData.value = [
                {
                    title: '营业', icon: '../../src/assets/img/q1.png',
                    count: data.BUSINESS
                },
                {
                    title: '供水', icon: '../../src/assets/img/q2.png',
                    count: data.WATER_SUPPLY
                },
                {
                    title: '漏水', count: data.WATER_LEAK, icon: '../../src/assets/img/q3.png',
                },
                { title: '排水', icon: '../../src/assets/img/q4.png',},
                { title: '工程', icon: '../../src/assets/img/q5.png',},
                { title: '服务', icon: '../../src/assets/img/q6.png',},
            ]
        }
        // 获取问题板块件数
        const getSectionCount = async () => {
            try {
                const response = await get('/shenyang-report/screen/questionSectionCount');
